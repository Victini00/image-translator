...
직접 파인튜닝한 인식 모델이 어느 수준인지, 상용 API와는 얼마나 차이가 나는지 측정했습니다.

**평가 방법** — 일본어 만화 페이지에서 파이프라인이 검출한 문단 71개가 대상으로, 정답은 이미지를 보고 사람이 확정했고, 세 시스템에 같은 문단을 넣어 CER(문자 오류율)과 완전일치율을 측정함

**구두점 포함**

| 시스템 | CER | 완전일치 |
|---|---|---|
| PP-OCRv5 원본 | 45.2% | 16.9% |
| PP-OCRv5 파인튜닝 (v4) | 29.9% | 11.3% |
| Gemini | 4.3% | 88.7% |
...
| 시스템 | CER | 완전일치 |
|---|---|---|
| PP-OCRv5 원본 | 43.2% | 22.9% |
| PP-OCRv5 파인튜닝 (v4) | **7.8%** | 45.7% |
| Gemini | 0.9% | 94.3% |

**1. 파인튜닝의 유효성**: 글자 기준 CER이 43.2% → 7.8%로 떨어졌습니다. 파인튜닝 후 사용 가능한 정확도에 도달했습니다.

**2. 남은 오류는 대부분 세로 조판 기호**: 파인튜닝 모델은 구두점을 빼면 7.8%지만 포함하면 29.9%로, **22%p가 「……」 오독에서 나옵니다.** 이는 학습 데이터셋의 문제로, 개선 예정입니다.
...
- **세로쓰기 특수문자 오독** — 학습 데이터에 세로 조판 기호가 없어 「……」 등을 잘못 읽습니다. 이 오류만으로 CER이 22%p 늘어납니다([Recognition Performance](#recognition-performance) 참고).
