"""
[v4까지 쓴 학습 데이터 생성기 - 현재는 사용하지 않음]

인식 모델 v4(100만 쌍)를 만들 때 쓴 모듈이다. TRDG를 subprocess로 불러 이미지를
생성했는데, 실제 만화와 맞지 않는 점이 몇 가지 확인되어 후속 버전부터는
manga_dataset_generator.py를 쓴다.

확인된 한계:
  - TRDG의 세로 모드(orientation=1)가 글자를 회전 없이 쌓기만 해서, 「ー」가 가로
    막대로, 「……」가 가로 3점 두 줄로 그려졌다. 실제 세로 조판과 다른 그림이라
    모델이 이 기호들을 배우지 못했다(v4 인식 오류의 78%가 말줄임표에서 발생).
  - 배경이 흰색 단색 고정(-b 1)이라 스크린톤 위 글자를 본 적이 없다.
  - 왜곡·블러·해상도 변화를 주지 않아 인쇄/스캔 열화에 취약하다.
  - 폰트가 3종뿐이었다.
